...
int main(void)
{
    ...
    uint32_t current_counter_1; //R
	uint32_t current_counter_2; //L
    uint32_t current_counter_3; //H
    uint32_t current_counter_4; //V

    uint32_t previous_counter_1 = 0;
	uint32_t previous_counter_2 = 0;
    uint32_t previous_counter_3 = 0;
    uint32_t previous_counter_4 = 0;

    double extinction_ratio_1; // (R + L)/L
    double extinction_ratio_2; // (R + L)/R
    double extinction_ratio_3; // (H + V)/V

    double resolution = 0.1;

    int arraySize = 50;
    int R_counts[arraySize];
    int L_counts[arraySize];
    int H_counts[arraySize];
    int V_counts[arraySize];
    ...
    int i;
    int j;
    //Get the channel counts and put them in array
    for (i = 0; i < arraySize; i++) {
        R_counts[i] = *(axi_pio_adr_reg_3);
        L_counts[i] = *(axi_pio_adr_reg_0);
        H_counts[i] = *(axi_pio_adr_reg_1);
        V_counts[i] = *(axi_pio_adr_reg_2);
        usleep(1000000*resolution);
    }

    // Calculate the sum of array elements
    for (j = 0; j < arraySize - 1; j++) {
        R_sum += R_counts[j+1] - R_counts[j];
        L_sum += L_counts[j+1] - L_counts[j];
        H_sum += H_counts[j+1] - H_counts[j];
        V_sum += V_counts[j+1] - V_counts[j];
    }
    // Calculate the average
    R_mean = (double)R_sum / arraySize / (resolution*1000000);
    L_mean = (double)L_sum / arraySize / (resolution*1000000);
    H_mean = (double)H_sum / arraySize / (resolution*1000000);
    V_mean = (double)V_sum / arraySize / (resolution*1000000);

    printf("#####################MEAN COUNTS####################\n");
    printf("Counts R = %lf MHz\n",R_mean);
    printf("Counts L = %lf MHz\n",L_mean);
    printf("Counts H = %lf MHz\n",H_mean);
    printf("Counts V = %lf MHz\n",V_mean);
    usleep(2000000);

    while(1){

        current_counter_1 = *(axi_pio_adr_reg_3);
        current_counter_2 = *(axi_pio_adr_reg_0);
        current_counter_3 = *(axi_pio_adr_reg_1);
        current_counter_4 = *(axi_pio_adr_reg_2);

        R = (double)(current_counter_1 - previous_counter_1)/(double)resolution/1000000 - R_mean;
        L = (double)(current_counter_2 - previous_counter_2)/(double)resolution/1000000 - L_mean;
        H = (double)(current_counter_3 - previous_counter_3)/(double)resolution/1000000 - H_mean;
        V = (double)(current_counter_4 - previous_counter_4)/(double)resolution/1000000 - V_mean;

        extinction_ratio_1 = (double)(R + L)/ (double)L;
        extinction_ratio_2 = (double)(R + L)/ (double)R;
        extinction_ratio_3 = (double)(H + V)/ (double)V;

        previous_counter_1 = current_counter_1;
        previous_counter_2 = current_counter_2;
        previous_counter_3 = current_counter_3;
        previous_counter_4 = current_counter_4;
